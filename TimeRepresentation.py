...
class TimeRepresentation:
    ...
    def predict(self, x, ptype='all', in_nstat=None):
        ...
        steady = np.zeros((self.nobs*nstat,))
        ...
        for k in range(nstat):
            ...
            if self._matrix is not None:
                if len(noreg_ind) > 0:
                    steady[k::nstat] = np.dot(self._matrix[:,noreg_ind], xi[noreg_ind])
                if len(reg_ind) > 0:
        ...
        outdict = {'steady': steady, 'seasonal': seasonal, 'transient': transient,
            'total': steady + seasonal + transient}
